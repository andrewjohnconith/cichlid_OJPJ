...
setwd("/Users/home/Dropbox/Amherst/Post-Doc/Muscles/OJ_PJ/Writing/MS/Writing/ReSubmission/FinalSubmission/GitHub/QTL/Scaffold/LOJ/PC2/")
...
for (i in 1:length(ScafNames)){
  ...
  TestingD<-as.data.frame(cbind(Scaffold0$pheno$HybridID,ScafID,Scaffold0$pheno$PC2))
...
for (i in 1:length(ScafNames)){ 
  ...
  TestingD<-as.data.frame(cbind(Scaffold21$pheno$HybridID,ScafID,Scaffold21$pheno$PC2))
  colnames(TestingD)<-c("ID","factor","value")
  r2<-ddply(TestingD, .(factor), summarize, mean=mean(value))
  r3<-ddply(TestingD, .(factor), summarize, mean=std.error(value))
...
plot(MapPPP$Pos, abs(MapPPP$AA.BB), ylim = range(abs(MapPPP$AA.BB)), type='l', lwd=2, axes=T, xlab=NA, ylab=NA, col="orange")
abline(h=0)
