...
LPJ_CichlidMeanShapes<-readland.tps(file = "MacroLPJ_Mean_20210518.tps", specID = "ID")
LOJ_CichlidMeanShapes<-readland.tps(file = "MacroLOJ_Mean_20210518.tps", specID = "ID")
...
p + geom_abline(intercept = pPLSCoefficients$coefficients[1], slope = pPLSCoefficients$coefficients[2]) +
  ...
  #geom_text(aes(label=TaxaID),hjust=0, vjust=0)
...
#LPJ
LPJPCAs<-gm.prcomp(A = LPJ_CichlidMeanShapes_Tr, phy = FTree, align.to.phy = F)
pc.plot<-plot(LPJPCAs, axis1 = 1, axis2 = 2, phylo = F, col=ColourVec[unclass(gp.Diet)], pch=ShapeVec[unclass(gp.Diet)])
shapeHulls(pc.plot, groups = as.character(gp.Diet), group.cols = c("#4daf4a", "#377eb8", "#ff7f00", "#984ea3","#a65628", "#e41a1c", "#ffff33"))
...
#M vs. T: p=0.24; M proc var = 0.07256773; T proc var = 0.05572648

#LPJ
LPJfit_Lake<-procD.pgls(f1 = LPJ_CichlidMeanShapes_Tr~gp.Lake, phy = FTree)
morphol.disparity(f1 = LPJfit_Lake)
#M vs. T: p=0.037; M proc var = 0.01571410; T proc var = 0.02182689*
